...
export default function Header({ currentView }) {
  ...
  return (
    <header className="relative z-50 flex flex-col md:flex-row justify-between items-center mb-2 gap-3 border-b border-gray-200/30 pb-2 w-full">
      {/* 로고 영역 (왼쪽 정렬) */}
      ...
      <div className="md:flex-1 flex items-center justify-center md:justify-end w-full md:w-auto">
        {user ? (
          <div className="relative group">
            ...
            <div className="absolute right-0 top-full mt-2 w-64 bg-[#FAF8F5] border border-[#4A5D4E]/15 rounded-xl shadow-xl overflow-hidden transition-all duration-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible translate-y-2 group-hover:translate-y-0 z-[999] flex flex-col">
              {/* 상단: 닉네임 & 로그아웃 */}
              ...
              <div className="p-4">
                {currentView === 'history' ? (
                  <button
                    onClick={() => goTo('focus')}
                    className="w-full py-2.5 rounded-xl bg-[#4A5D4E] hover:bg-[#3d4c40] text-[#F9F6F0] text-center text-xs font-semibold shadow transition cursor-pointer select-none"
                  >
                    타이머 화면
                  </button>
                ) : (
