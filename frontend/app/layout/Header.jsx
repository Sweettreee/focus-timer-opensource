...
import { useNavController } from "../useNavContoller";
...
export default function Header({ currentView }) {
  ...
  return (
    <header className="relative z-50 flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 bprder-b border-gray-200/50 pb-4">
      <div>
        <div
          ...
          title="홈 화면으로 이동"
        >
          <h1 className="text-2xl font-bold tracking-wider text-[#4A5D4E]">
            FOCUS ROOM
          </h1>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-3 md:gap-4">
        <div className="flex items-center gap-5 md:gap-7 select-none px-2">
          ...
          <div className="text-center">
            ...
            <p className="font-serif italic font-demium text-gray-800 text-[16px] mt-0.5">
              {stats.streak}d
            </p>
          </div>
        </div>
        ...
        {user ? (
          <div className="relative group">
            {/* 사람이모티콘 My 버튼 */}
            <button
              className="flex items-center gap-1.5 px-3.5 py-2.5 bg-white/80 border border-[#4A5D4E] hover:bg-[#4A5D4E]/5 text-[#4A5D4E] font-semibold text-xs tracking-wide rounded-xl shadow-sm hover:shadow transition-all duration-200 cursor-pointer select-none"
              title="마이 메뉴"
            >
            ...
            <div className="absolute right-0 top-full mt-2 w-64 bg-[#FAF8F5] border border-[#4A5D4E]/15 border-t-[3px] border-t-[#D27D66] rounded-xl shadow-xl overflow-hidden transition-all duration-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible translate-y-2 group-hover:translate-y-0 z-[999] flex flex-col">
              ...
              <div className="flex justify-between items-center px-4 py-3 border-b border-[#4A5D4E]/10 bg-[#F5F2EC]/30">
                <span className="font bold text-gray-800 text-sm truncate max-w-[130px]">
                  {user.nickname}
                </span>
                <button
                  onClick={logout}
                  className="px-2 py-1 text-[10px] font-bold text-gray-500 hover:text-red-600 bg-gray-200/50 hover:bg-red-50 roundede-lg transition-colors cursor-pointer"
                >
                  로그아웃
                </button>
              </div>
              ...
              <div className="p-4 flex flex-col gap-2.5">
                <div className="flex justify-between items-center text-xs">
                  ...
                  <span className="font-serif italic font-semibold text-[#4A5D4E]">
                    {stats.todayFocus}
                  </span>
                </div>
                <div className="flex justify-between items-center text-xs">
          ...
          <div className="flex gap-1">
            <button
              onClick={() => goTo("login")}
              className="flex items-center gap-1.5 px-3 py-2 bg-[#6B8E23]/10 hover:bg-[#6B8E23]/20 border border-[#6B8E23]/20 hover:border-[#6B8E23]/30 text-[#4A5D4E] transition-all rounded-xl text-xs font-semibold trakcing-wide cursor-pointer sadow-sm"
              title="로그인"
            >
