...
export default function FaceCamera({aiEnabled,isAiPaused,onFaceLost,onFaceReturned}){
    ...
    const [isModeLoaded,setIsModelLoaed] = useState(false);//인공지능 모델이 다운로드 완료 되었는지 아직 로딩중인지
    ...
    //최신 정보를 실시간으로 기록 
    const propRef = useRef({aiEnabled,isAipaused,onFaceLost,onFaceReturned});
    //버튼 누르면 카메라 전원 켜기 
    useEffect(()=>{
    ...
    // 1. MediaPipe 모델 초기화
    useEffect(()=>{
        const initModel = async () =>{//async를 통해서 await를 사용
            try{
                const vision = await FilesetResolver.forVisionTasks(
                    "https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@latest/wasm"); //인공지능이 계산을 할 수 있도록 기반환경을 구글 서버에서 받아옴
    ...
    const predictWebcam = ()=>{
        //카메라가 꺼져있다면 즉시 루프 중단 
        if(!webcamActive) retrun;
        //모델이랑 영상 태그가 다 로드되었을 때만 루프를 실행하도록 한다 
        if(detectorRef.current&&videoRef.current){//ai분석기 준비되었는지 확인 및 비디오 장치 켜져 있는지 확인 
            try{
                if (videoRef.current.currentTime !== lastVideoTimeRef.current) {//비디오의 현재 재생 시간과 직전에 검사했던 시간 비교
            ...
            if(detection && detections.detections.length >0){
                setIsFaceDetected(true);
                faceLostTimeRef.current = 0 ; //이탈 타이머 누적 리셋 
                if (propsRef.current.aiEnabled && propsRef.current.isAiPaused) {//ai기능 켜져 있고 사용자가 자리 멈춘상태 
                    if(faceReturnedTimeRef.current===null||faceReturnedTimeRef.current === 0){//스톱워치가 처음 켜진상태이거나,쓰다 리셋된 상태
