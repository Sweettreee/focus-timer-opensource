...
// 표시 중인 달의 그리드 셀 배열 생성 (앞쪽 빈 칸 포함)
...
export default function HistoryPage() {
  ...
  const realSessions = useSessions();
  const { deleteSession } = useSessionController();
  ...
  // 실데이터만 사용(목업 없음). 기록이 없으면 빈 상태를 그대로 보여준다.
  const decorated = realSessions.map(decorateSession);
  ...
  const daySessions = decorated
    ...
    .sort((a, b) => a.timestamp - b.timestamp); // 시간 오름차순
  // 선택한 날짜 세션들의 총 공부 시간
  ...
  const dayHeading = selectedDateObj
    .toLocaleDateString([], { month: "long", day: "numeric", weekday: "long" })
    .toLowerCase();
  ...
  return (
    <div className="min-h-screen bg-[#F9F6F0] text-[#333333] p-6 flex flex-col justify-between font-sans selection:bg-[#6B8E23]/20">
      {/* 상단 헤더 영역 (메인 대시보드 네비게이션바 그대로 반영) */}
