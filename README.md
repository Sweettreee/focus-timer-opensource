...
- **AI Vision**: MediaPipe Tasks Vision (`blaze_face_short_range` 모델)
...
- 하단 토글을 통해 웹캠 기능을 끄고 **일반 타이머 모드**로도 사용 가능

### 2. 식물 성장 시각화 및 세션 기록
- 설정 시간(2분, 25분, 50분 등)의 진행도 비율에 맞춰 **화면 내 식물 에셋이 성장**하도록 시각화
...
> 서버 시작 시 데이터베이스 스키마와 테이블은 **자동으로 생성 및 초기화**됩니다.
...
루트 디렉토리에서 아래 명령어를 실행하면 프론트엔드와 백엔드를 한 번에 세팅하고 동시 구동할 수 있습니다.
